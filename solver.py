...
class Piece:
    ...
    def __init__(self, index, piece):
        ...
        self.index = index
        piece = cv.cvtColor(piece, cv.COLOR_BGR2GRAY)  # Using grayscale for easier comparison later on
    ...
    def get_edge_values(self):
        for value in self.top_edge:
            self.top_edge_value = self.top_edge_value + value
        for value in self.bottom_edge:
            self.bottom_edge_value = self.bottom_edge_value + value
        for value in self.right_edge:
            self.right_edge_value = self.right_edge_value + value
        for value in self.left_edge:
            self.left_edge_value = self.left_edge_value + value
...
class Generation:
    ...
    def fitness(self, solution: list[Piece]) -> Tuple[float, list]:
        ...
        for index in range(len(solution)):  # pieces
            if (index + 1) % self.cols != 0:
                ...
                if index + 1 <= self.cols * (
                    ...
                    # edge_likelihood = edge_likelihood + self.compare_edges(solution[index].bottom_edge,
                    #                                                        solution[index + self.cols].top_edge)
                    # edge_likelihood = edge_likelihood + self.compare_edges(solution[index].right_edge,
                    #                                                        solution[index + 1].left_edge)
                ...
                if index + 1 <= self.cols * (self.rows - 1):
                    # edge_likelihood = self.compare_edges(solution[index].bottom_edge,
                    #                                      solution[index + self.cols].top_edge)
    ...
    def make_pieces(self):  # gets the pieces from the image
        for i in range(self.rows):
            for j in range(self.cols):
                self.pieces.append(Piece(i * self.rows + j, self.image[i * self.row_size:(i + 1) * self.row_size, j * self.col_size:(j + 1) * self.col_size]))
    ...
    def start_generation(self):  # main functionality
        ...
        for gen in range(
            ...
            for j in range(mutated_num):
                new_solutions.append(Evolution.mutate(ranked[random.randint(0, 99)][1], max(1, gen//mutation_tolerance)))
...
# 10, 10, 200, 500, 20
algorithm = GeneticAlgorithm(img, rows=10, cols=10, sol_num_per_generation=700, generation_num=4000, edge_threshold=20)
